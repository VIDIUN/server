	...
	private static function getWriter(Zend_Config $config)
	{
		...
		switch($config->name)
		{
			case "Zend_Log_Writer_Stream":
				...
				$writer = new VidiunSerializableStream($config->stream, $mode);
				break;
		}
	...
	private static function getFilter(Zend_Config $config)
	{
		...
		switch($config->name)
		{
			...
			case "Zend_Log_Filter_Suppress":
				...
				break;
			case "VidiunLogPartnerFilter":
				if ($config->partnerId !== null)
				{
					$filter = new VidiunLogPartnerFilter($config->partnerId);
				}
				break;
			case "VidiunLogFilterType":
				if ($config->type !== null)
				{
					$filter = new VidiunLogFilterType($config->type);
				}
				break;
		}
		return $filter;
