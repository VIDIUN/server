 ...
 * @subpackage Media
 */
class VFFMpegMediaParser extends VBaseMediaParser
{
	protected $cmdPath;
	...
	public function __construct($filePath, $ffmpegBin=null, $ffprobeBin=null)
	{
		...
		else if(vConf::hasParam('bin_path_ffmpeg')) {
			$this->cmdPath = vConf::get('bin_path_ffmpeg');
		}
		else{
			$this->cmdPath = "ffmpeg";
		}
		...
		else if (vConf::hasParam('bin_path_ffprobe')) {
			$this->ffprobeBin = vConf::get('bin_path_ffprobe');
		}
		else{
...
//		list($silenceDetect, $blackDetect) = self::checkForSilentAudioAndBlackVideo($this->cmdPath, $this->filePath, $mediaInfo);
		...
		VidiunLog::log(print_r($mediaInfo,1));
	 ...
	 * @return VidiunMediaInfo
	 */
	protected function parseFormat($format, VidiunMediaInfo $mediaInfo)
	{
	 ...
	 * @return VidiunMediaInfo
	 */
	protected function parseStreams($streams, VidiunMediaInfo $mediaInfo)
	{
	$vidCnt = 0;
	$audCnt = 0;
	$dataCnt = 0;
	 ...
	 * @return VidiunMediaInfo
	 */
	protected function parseVideoStream($stream, VidiunMediaInfo $mediaInfo)
	{
	...
	{
		$lout = VDLAudioLayouts::Detect($layout);
		if(!isset($lout))
			$lout = $layout;
		return $lout;
	}
	 ...
	 * @return VidiunMediaInfo
	 */
	protected function parseDataStream($stream, VidiunMediaInfo $mediaInfo)
	{
	 ...
	 */
	public static function checkForSilentAudioAndBlackVideo($ffmpegBin, $srcFileName, VidiunMediaInfo $mediaInfo, $detectDur=null)
	{
		VidiunLog::log("contDur:$mediaInfo->containerDuration,vidDur:$mediaInfo->videoDuration,audDur:$mediaInfo->audioDuration");
		...
		if(empty($detectMsg))
			VidiunLog::log("No black frame or silent content in $srcFileName");
		else
			VidiunLog::log("Detected - $detectMsg, in $srcFileName");
	 ...
	 * @return boolean
	 */
	public static function checkForGarbledAudio($ffmpegBin, $srcFileName, VidiunMediaInfo $mediaInfo)
	{
		VidiunLog::log("contDur:$mediaInfo->containerDuration,audDur:$mediaInfo->audioDuration");
		...
		if($audDetectDur>0 && $audDetectDur<10){
			VidiunLog::log("Audio OK - short audio, audDetectDur($audDetectDur)");
			return false;
		}
		
		list($silenceDetected,$blackDetected) = VFFMpegMediaParser::detectSilentAudioAndBlackVideoIntervals($ffmpegBin, $srcFileName, null, 0.05, $audDetectDur,"-90dB");
		
		$ticks = isset($silenceDetected)? count($silenceDetected): 0;
		if($ticks<=10){
			VidiunLog::log("Audio OK - low numbers of ticks($ticks)");
			return false;
		}
		
		VidiunLog::log("audDetectDur($audDetectDur),ticks($ticks)");
		if($audDetectDur>0) {
			$ticksPerMin = $ticks/($audDetectDur/60);
			VidiunLog::log("ticksPerMin($ticksPerMin)");
			
			if($ticksPerMin<15 
			||($audDetectDur<60 && $ticksPerMin<30) 
			||($audDetectDur<120 && $ticksPerMin<20) ){
				VidiunLog::log("Audio OK");
				return false;
			}
		}
		else if($ticks<100) {
			VidiunLog::log("Audio OK - no duration, number of ticks smaller than threshold(100)");
			return false;
		}
		
		VidiunLog::log("Detected garbled audio.");
		return true;
	}
	...
	{
		//		VidiunLog::log("checkSilentAudioAndBlackVideo(contDur:$mediaInfo->containerDuration,vidDur:$mediaInfo->videoDuration,audDur:$mediaInfo->audioDuration)");
		...
		*/
		...
		if(empty($detectFiltersStr)){
			VidiunLog::log("No duration values in the source file metadata. Cannot run black/silence detection for the $srcFileName");
			return null;
		}
		...
		VidiunLog::log("Black/Silence detection cmdLine - $cmdLine");
		...
		*/
		...
		if($rv!=0) {
			VidiunLog::err("Black/Silence detection failed on ffmpeg call - rv($rv),lastLine($lastLine)");
			return null;
		}
	...
	{
		VidiunLog::log("srcFileName($srcFileName)");
		...
		$lastLine=exec($cmdLine , $outputArr, $rv);
		if($rv!=0) {
			VidiunLog::err("SceneCuts detection failed on ffmpeg call - rv($rv),lastLine($lastLine)");
			return null;
		}
		/*
	...
	public static function retrieveKeyFrames($ffprobeBin, $srcFileName,$start=null,$duration=null)
	{
		...
		$lastLine=exec($cmdLine , $outputArr, $rv);
		if($rv!=0) {
			VidiunLog::err("Key Frames detection failed on ffmpeg call - rv($rv),lastLine($lastLine)");
			return null;
		}
		return $outputArr;
	}
	...
	{
		$vFrames = VFFMpegMediaParser::retrieveKeyFrames($ffprobeBin, $srcFileName, $start, $duration);
		if(!isset($vFrames) || count($vFrames)<2){
			return null;
		}
			/*
			 * Turn the KF timings into integers representing 10th seconds
			 */
		foreach($vFrames as $k=>$kF){
			$vFrames[$k] = (int)round($kF*100);
		}
VidiunLog::log("VFrames:".serialize($vFrames));
			/*
			 * Calculate GOP minimum, maximum and histogram counters
			 */
		$gopMin = $gopMax = $vFrames[1]-$vFrames[0];
...
//		$gopHist[$gopMin] = 1;
		...
		for($i=2;$i<count($vFrames); $i++){
			$gop = $vFrames[$i]-$vFrames[$i-1];
...
/*			if(key_exists($gop, $gopHist)){
		...
		$delta=6;
		for($tm=$vFrames[0]; $tm<=$vFrames[count($vFrames)-1];$tm+=50){
			for($t=$tm-$delta; $t<=$tm+$delta; $t++){
				if(array_search($t, $vFrames)!==false){
					break;
				}
			}
			if($t>$tm+$delta){
...
VidiunLog::log("kf2gopHist raw:".serialize($kf2gopHist));
			/*
			 * Calculate the appeared-to-expected-number-of-GOPs ratio.
			 */ 
		foreach($kf2gopHist as $gop=>$cnt) {
			$kf2gopHist[$gop] = $cnt/(round(($vFrames[count($vFrames)-1]-$vFrames[0])/$gop-0.5)+1);
		}
			// Sort the histogram array and get the GOP value that had the higest ratio
		asort($kf2gopHist);
VidiunLog::log("kf2gopHist norm:".serialize($kf2gopHist));
		end($kf2gopHist);
...
*/
		...
		VidiunLog::log("ScanType detection cmdLine - $cmdLine");
		$lastLine=exec($cmdLine , $outputArr, $rv);
		if($rv!=0) {
			VidiunLog::err("ScanType detection failed on ffmpeg call - rv($rv),lastLine($lastLine)");
			return 0;
		}
		...
		if($progessive<$tff+$bff)
			$scanType = 1;
		VidiunLog::log("ScanType: $scanType");
		return $scanType;
	}
...
*/
		...
		VidiunLog::log("FastStart detection cmdLine - $cmdLine");
		$lastLine=exec($cmdLine, $outputArr, $rv);
		{
			VidiunLog::log("FastStart detection results printout - lastLine($lastLine),output-\n".print_r($outputArr,1));
		}
		$fastStart = 1;
		foreach($outputArr as $line){
		...
		VidiunLog::log("FastStart: $fastStart");
		return $fastStart;
/*		
...
*/
	...
	public static function retrieveVolumeLevels($ffprobeBin, $srcFileName, $reset=1)
	{
		...
		$lastLine=exec($cmdLine , $outputArr, $rv);
		if($rv!=0) {
			VidiunLog::err("Volume level detection failed on ffprobe call - rv($rv),lastLine($lastLine)");
			return null;
		}
		$volumeLevels = array();
		foreach($outputArr as $line) {
