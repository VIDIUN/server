 ...
 * @subpackage Email-Ingestion
 */
class VAsyncEmailIngestion extends VPeriodicWorker
{
	/***************/
	/* Definitions */
	/***************/
	...
	{
		return VidiunBatchJobType::EMAIL_INGESTION;
	}
	...
	*/
	public function run($jobs = null)
	{
		...
		try {
			...
			VidiunLog::crit("Cannot find all required parameters from config file");
		}

		// create a temp file path
		if ( !self::createDir( $this->TEMP_FILE_DIR ) )
		{
			VidiunLog::crit( "Cannot continue email ingestion without a temp directory");
		...
		while ( isset(self::$taskConfig->params->{'mailbox'.($mailboxNumber+1)}) ) {
			...
			try {
				...
				VidiunLog::crit("Cannot find all required parameters from config file for mailbox number [$mailboxNumber]");
				continue; // skip current mailbox
			}


			// connect to current mailbox
			$mailChecker = new VMailChecker($host, $port, $user, $pass, $options);
			if (!$mailChecker->connect()) {
				VidiunLog::crit("Error connecting to [$host:$port] as [$user] - ".imap_last_error());
				continue; // skip current mailbox
			}
			VidiunLog::info("Sucessfuly connected to [$host:$port] as [$user]");
			...
			if (!$newMails || count($newMails) <= 0) {
				...
				VidiunLog::info("No new mails found on [$user@$host]");
				continue; // skip current mailbox
			}
			VidiunLog::info('['.count($newMails)."] unread mails found on [$user@$host]");
			...
			while ($keepCurMailbox && (list(,$curId) = each($newMails))) {
				...
				if ($mailesProcessed >= $maxMails) {
					VidiunLog::info("Reached the max mails per job for current mailbox [$mailboxNumber] - skipping to next mailbox");
				...
				if (!$curMail) {
					VidiunLog::err("Error fetching message with folder ID [$curId] from [$user@$host] - ".imap_last_error());
				...
				if (!$curMail->attachments || count($curMail->attachments) == 0) {
					...
					VidiunLog::info('No attachments found for mail ['.$curMail->header->msgid."] on [$user@$host] from [".$curMail->header->fromadd.'] with subject ['.$curMail->header->subject.']');
					if (!$mailChecker->moveMsg($curId, self::NO_ATTACHMENT)) {
						VidiunLog::err('Failed moving msg ['.$curMail->header->msgid.'] to the ['.self::NO_ATTACHMENT.'] folder - '.imap_last_error());
					}
				...
				if (!$email_profiles) {
					...
					VidiunLog::err('Partner validation failed for ['.$curMail->header->msgid."] on [$user@$host] from [".$curMail->header->fromadd.'] with subject ['.$curMail->header->subject.']');
					if (!$mailChecker->moveMsg($curId, self::PARTNER_INVALID)) {
						VidiunLog::err('Failed moving msg ['.$curMail->header->msgid.'] to the ['.self::PARTNER_INVALID.'] folder - '.imap_last_error());
					}
				...
				foreach ($email_profiles as $profile) {		
					VidiunLog::info("*** Currently processing attachments for email profile id [$profile->id] of partner id [$profile->partnerId]");
					...
					if (!$this->addEntries($curMail, $profile, $mediaEntry, $failures)) {
						VidiunLog::err("Some errors occured while adding entries for email profile id [$profile->id] of partner id [$profile->partnerId]");					
					}
				}
				...
				if ($failures->problemsHappened()) {
					...
					if ($failures->upload_failed || $failures->add_entry_failed || $failures->error_saving_temp_file) {
						...
						VidiunLog::crit('Failed adding some attachments for ['.$curMail->header->msgid."] on [$user@$host] Moving msg to [".$new_folder.'] folder');
					}
					else if ($failures->too_many_attachments) {
						// too many attachments
						$new_folder = self::INCOMPLETE;
						VidiunLog::err('Msg ['.$curMail->header->msgid."] on [$user@$host] from [".$curMail->header->fromadd.'] with subject ['.$curMail->header->subject."] contains too many attachments. Moving msg to [$new_folder] folder");
					}
						...
						if (count($curMail->attachments) > 1) {
							$new_folder = self::INCOMPLETE;
							VidiunLog::err('Some invalid attachments found for msg ['.$curMail->header->msgid."] on [$user@$host] from [".$curMail->header->fromadd.'] with subject ['.$curMail->header->subject."]. Moving msg to [$new_folder] folder");
						}
						else {
							$new_folder = self::ATTACHMENT_INVALID;
							VidiunLog::err('Msg attachment is invalid for msg ['.$curMail->header->msgid."] on [$user@$host] from [".$curMail->header->fromadd.'] with subject ['.$curMail->header->subject."]. Moving msg to [$new_folder] folder");
						}
					}
					else {
						// shouldn't get here
						VidiunLog::err('*** Not all addEntriesFailures situations were handled.');
					}
					
					// move msg to the right error folder
					if (!$mailChecker->moveMsg($curId, $new_folder)) {
						VidiunLog::err('Failed moving msg ['.$curMail->header->msgid.'] to the ['.$new_folder.'] folder - '.imap_last_error());
					}			
				}
					...
					if (!$mailChecker->moveMsg($curId, self::PROCESS_OK)) {
						VidiunLog::err('Msg ['.$curMail->header->msgid.'] from ['.$curMail->header->fromadd.'] with subject ['.$curMail->header->subject.'] was processed OK but failed moving to the ['.self::PROCESS_OK.'] folder - '.imap_last_error());
					}
				}
	...
	private function validePartnerAndGetProfile($toAddresses, $mailbox)
	{
		...
		try {
			...
			// problem
			VidiunLog::err('There was an error getting email profiles from the server - '.$e->getMessage());
	 ...
	 * @param VidiunEmailIngestionProfile $profile
	 * @param VidiunMediaEntry $mediaEntry media entry with data from mail's body
	 * @param AddEntriesFailures $failures
	 * @return <bool> true/false according to success
	 */
	private function addEntries(MailMsg &$mailData, VidiunEmailIngestionProfile $profile, VidiunMediaEntry $mediaEntry, AddEntriesFailures $failures)
	{
		$problems_happened = false;
		$entry_name = $mediaEntry->name;
		
		self::$vClient->setPartnerId($profile->partnerId);
		...
		foreach ($mailData->attachments as $cur_attach) {
			...
			if ($profile->maxAttachmentsPerMail && ($num > $profile->maxAttachmentsPerMail)) {
				VidiunLog::info ('Mail msg ['.$mailData->header->msgid.'] has more than ['.$profile->maxAttachmentsPerMail.'] attachments - ignoring the rest.');
			...
			if (!$this->validateAttachment($cur_attach, $errorMsg)) {
				...
				VidiunLog::err("Attachment [{$cur_attach->filename}] is not valid - $errorMsg");
			...
			VidiunLog::info("Attachment [{$cur_attach->filename}] is temporarly saved with name [$filename]");
			...
			if (!$fileWritten) {
				VidiunLog::err("Error writing to [$filename] in the temp directory.");
			...
			if ($profile->maxAttachmentSizeKbytes && (vFile::fileSize($filename)/1024 > $profile->maxAttachmentSizeKbytes)) {
				VidiunLog::info("Attachment [$cur_attach->filename] is too big for profile [$profile->id] - ignoring.");
				// delete the temporary file from the disk			
				if (!unlink($filename)) {
					VidiunLog::info("Cannot delete [$filename] from the temp directory");
				}
			...
			// upload file to the vidiun server
			// ---------------------------------
			try {
			...
			if ($token->id == null || !$token->id) {
				VidiunLog::err("Error uploading [$filename] to the vidiun server.");
				$problems_happened = true;
			...
			if ($newEntry == null || !$newEntry) {
				VidiunLog::err("Error adding entry from uploaded file [$filename], token  [".$token->id."].");
				$problems_happened = true;
			...
			VidiunLog::info("New entry added succesfully with id [{$newEntry->id}]");
			$num++;
    ...
    private function createUploadTokenAndUpload ($profile, $filename)
    {
	    ...
	    $uploadToken->fileSize = vFile::fileSize($filename);
