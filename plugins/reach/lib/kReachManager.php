 ...
 */
class vReachManager implements vObjectChangedEventConsumer, vObjectCreatedEventConsumer, vObjectAddedEventConsumer, vGenericEventConsumer
{
	/**
	...
	*/
	...
	private function handleEntryReady(entry $object)
	{
		...
		foreach ($pendingEntryReadyTasks as $pendingEntryReadyTask)
		{
			...
			$pendingEntryReadyTask->setAccessKey(vReachUtils::generateReachVendorVs($pendingEntryReadyTask->getEntryId(), $pendingEntryReadyTask->getIsRequestModerated(), $pendingEntryReadyTask->getCatalogItem()->getVsExpiry()));
	...
	public static function addEntryVendorTask(entry $entry, ReachProfile $reachProfile, VendorCatalogItem $vendorCatalogItem, $validateModeration = true, $version = 0, $context = null, $creationMode = null)
	{
		...
		$entryVendorTask->setAccessKey(vReachUtils::generateReachVendorVs($entryVendorTask->getEntryId(), $shouldModerateOutput, $vendorCatalogItem->getVsExpiry()));
