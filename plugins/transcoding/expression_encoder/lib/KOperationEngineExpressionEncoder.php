 ...
 * @subpackage lib
 */
class VOperationEngineExpressionEncoder  extends VSingleOutputOperationEngine
{
	/**
	 * @var string
	 */
...
//	protected $configData;
	...
	protected function getCmdLine()
	{
		...
		VidiunLog::info("outFilePath(dirname($this->outFilePath)),auxPath($outDir)");
	...
	{	
			 ...
			 * The code catches the Exceptions thrown by the VOperrationEngine. 
		...
		for($iEx=0; $iEx<5; $iEx++) {
			try {
				...
				VidiunLog::info("EE got hang up. Waiting $secsToSleep sec for next attempt. Attempt:$iEx");
				sleep($secsToSleep);
			}
		}
		throw new VOperationEngineException($ex->getMessage(), $ex->getCode());
	}
