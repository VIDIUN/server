...
class AESEncrypt
{
	...
	public static function encrypt($key, $message)
	{
		...
		return array(VCryptoWrapper::encrypt_aes($message, $key, $iv), $iv);
	}
	...
	public static function decrypt($key, $message, $iv)
	{
		$key = substr(sha1($key, true), 0, 16);
		/** @noinspection PhpUndefinedMethodInspection */
		return rtrim(VCryptoWrapper::decrypt_aes($message, $key, $iv), "\0");
	}
	...
	private static function genIv()
	{
		...
		do
		{
			...
			{
				throw new VidiunAPIException('Unable to genereate secure iv for tokens.');
			}
		} while (!$wasItSecure);
		return $iv;
	}
