 ...
 * @subpackage lib
 */
class vElasticSearchManager implements vObjectReadyForIndexEventConsumer, vObjectReadyForElasticIndexEventConsumer, vObjectUpdatedEventConsumer, vObjectAddedEventConsumer, vObjectChangedEventConsumer
{
    ...
    public function objectReadyForIndex(BaseObject $object, BatchJob $raisedJob = null)
    {
        ...
        {
            vCuePointManager::reIndexCuePointEntry($object, false, true);//reindex the entry only on elastic
        }
    ...
    {
        if(vConf::get('disableElastic', 'elastic', true))
            return true;
    ...
    private function checkRepetitiveUpdates($object)
    {
        ...
        // threshold is defined by vConf parameter skip_elastic_repetitive_updates using lowercase of {className}_{service}_{action}={threshold}
        $saveCounter = 0;
        ...
        list($skipElasticRepetitiveUpdatesValue, $matchKey) = vSearchUtils::getSkipRepetitiveUpdatesValue(self::REPETITIVE_UPDATES_CONFIG_KEY, $className);
        $skipSave = isset($skipElasticRepetitiveUpdatesValue) && $saveCounter > $skipElasticRepetitiveUpdatesValue;

        if($skipSave)
        {
            VidiunLog::debug("Skipping save elastic for object [$className] [$objectId] count [$saveCounter] max allowed [$skipElasticRepetitiveUpdatesValue] with match key [$matchKey]");
            return true;
        }
	
		VidiunLog::debug("Updating elastic for object [$className] [$objectId] count [$saveCounter] service info [" . vCurrentContext::$service . ' ' . vCurrentContext::$action . "]");
		return false;
    }
    ...
    public function getElasticSaveParams($object, $params)
    {
        ...
        $pluginInstances = VidiunPluginManager::getPluginInstances('IVidiunElasticSearchDataContributor');
        ...
        {
            VidiunLog::debug("Loading $pluginName elastic data contribution");
    ...
    public function execElastic($params, IElasticIndexable $object, $action)
    {
        ...
        try
        {
            ...
            if(!$ret)
            {
                VidiunLog::err('Failed to Execute elasticSearch query: '.print_r($params,true));
                return false;
            }
        }
        catch (Exception $e)
        {
            VidiunLog::warning('Failed to execute elastic');
        }
    ...
    {
        $key = vCurrentContext::$vs_partner_id . "_" . vCurrentContext::$service . "_" . vCurrentContext::$action . "_" . $object->getElasticIndexName();
        $map = vConf::get('partner_actions_to_skip_elastic_map', 'local', array());
        if (isset($map[$key]))
        {
            VidiunLog::log("Specific partner action to skip elastic detected $key. skipping elastic sync.");
            return false;
        }

        if (vConf::get('exec_elastic', 'local', 0))
            return true;

        if (vConf::hasParam('exec_elastic_client_tags'))
        {
            $execElasticTags = vConf::get('exec_elastic_client_tags');
    ...
    private function shouldSkipSaveToSphinxLog($object, &$command)
    {
        ...
        if (strlen($command) > self::MAX_SQL_LENGTH)
        {
            ...
            $cache = vCacheManager::getSingleLayerCache(vCacheManager::CACHE_TYPE_ELASTIC_STICKY_SESSIONS);
            if ($cache && !$cache->add($lockKey, true, 60))
            {
                VidiunLog::log('skipping saving elastic sphinxLog sql for key ' . $lockKey);
                return true;
            }
        }
        return false;
    ...
    private function retrieveElasticClusterId()
    {
        ...
        $elasticClusterName = vConf::get('elasticCluster', 'elastic', 0);
        $elasticClusterCacheStore = vCacheManager::getSingleLayerCache(vCacheManager::CACHE_TYPE_ELASTIC_EXECUTED_CLUSTER);
    ...
    {
        //prevent indexing 2 times- if object is IIndexable we raise the event in vSphinxSearchManager
    ...
    public function shouldConsumeChangedEvent(BaseObject $object, array $modifiedColumns)
    {
        ...
        $fieldsToMonitor = array(entryPeer::STATUS, entryPeer::VUSER_ID, entryPeer::CATEGORIES_IDS);
        // custom data for entitled vusers edit/publish
        $customDataFieldsToMonitor = array('entitledUserPuserEdit', 'entitledUserPuserPublish', 'creatorVuserId');
        $namespace = '';

        if($object instanceof entry)
        {
            unset($modifiedColumns[vObjectChangedEvent::CUSTOM_DATA_OLD_VALUES]);
    ...
    private function trimParamFields($tempParams)
    {
        ...
        {
            if (array_key_exists($item, $params) && (strlen($params[$item]) > vElasticSearchManager::MAX_LENGTH))
