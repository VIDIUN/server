 ...
 */
class VidiunHuluDistributionJobProviderData extends VidiunConfigurableDistributionJobProviderData
{
	/**
	 * @var string
	 */
	...
	public function __construct(VidiunDistributionJobData $distributionJobData = null)
	{
		...
		if(!($distributionJobData->distributionProfile instanceof VidiunHuluDistributionProfile))
			return;
		...
		$this->captionLocalPaths = new VidiunStringArray();
			...
			{	
				...
				if(vFileSyncUtils::fileSync_exists($syncKey)){
					if (($assetType == CaptionPlugin::getAssetTypeCoreValue(CaptionAssetType::CAPTION))||
						...
						$string->value = vFileSyncUtils::getLocalFilePathForKey($syncKey, false); 
