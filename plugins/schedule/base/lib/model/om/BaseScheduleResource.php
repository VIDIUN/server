...
/**
 * Base class that represents a row from the 'schedule_resource' table.
 *
 * 
 *
 ...
 * @subpackage model.om
 */
abstract class BaseScheduleResource extends BaseObject  implements Persistent {
	...
	{
		if(!isset($this->oldColumnsValues[ScheduleResourcePeer::ID]))
			$this->oldColumnsValues[ScheduleResourcePeer::ID] = $this->id;
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = ScheduleResourcePeer::ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ScheduleResourcePeer::PARENT_ID]))
			$this->oldColumnsValues[ScheduleResourcePeer::PARENT_ID] = $this->parent_id;
		...
		if ($this->parent_id !== $v) {
			$this->parent_id = $v;
			$this->modifiedColumns[] = ScheduleResourcePeer::PARENT_ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ScheduleResourcePeer::PARTNER_ID]))
			$this->oldColumnsValues[ScheduleResourcePeer::PARTNER_ID] = $this->partner_id;
		...
		if ($this->partner_id !== $v) {
			$this->partner_id = $v;
			$this->modifiedColumns[] = ScheduleResourcePeer::PARTNER_ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ScheduleResourcePeer::NAME]))
			$this->oldColumnsValues[ScheduleResourcePeer::NAME] = $this->name;
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = ScheduleResourcePeer::NAME;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ScheduleResourcePeer::DESCRIPTION]))
			$this->oldColumnsValues[ScheduleResourcePeer::DESCRIPTION] = $this->description;
		...
		if ($this->description !== $v) {
			$this->description = $v;
			$this->modifiedColumns[] = ScheduleResourcePeer::DESCRIPTION;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ScheduleResourcePeer::TAGS]))
			$this->oldColumnsValues[ScheduleResourcePeer::TAGS] = $this->tags;
		...
		if ($this->tags !== $v) {
			$this->tags = $v;
			$this->modifiedColumns[] = ScheduleResourcePeer::TAGS;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ScheduleResourcePeer::TYPE]))
			$this->oldColumnsValues[ScheduleResourcePeer::TYPE] = $this->type;
		...
		if ($this->type !== $v) {
			$this->type = $v;
			$this->modifiedColumns[] = ScheduleResourcePeer::TYPE;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ScheduleResourcePeer::STATUS]))
			$this->oldColumnsValues[ScheduleResourcePeer::STATUS] = $this->status;
		...
		if ($this->status !== $v) {
			$this->status = $v;
			$this->modifiedColumns[] = ScheduleResourcePeer::STATUS;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = ScheduleResourcePeer::CREATED_AT;
			}
	...
	public function setUpdatedAt($v)
	{
		...
		if ( $this->updated_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = ScheduleResourcePeer::UPDATED_AT;
			}
		} // if either are not null
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				ScheduleResourcePeer::doDelete($this, $con);
				$this->postDelete($con);
		...
		try {
			...
			for ($retries = 1; $retries < VidiunPDO::SAVE_MAX_RETRIES; $retries++)
			{
               $affectedRows = $this->doSave($con);
                if ($affectedRows || !$this->isColumnModified(ScheduleResourcePeer::CUSTOM_DATA)) //ask if custom_data wasn't modified to avoid retry with atomic column 
                	break;
				...
				$validUpdate = true;
				$atomicCustomDataFields = ScheduleResourcePeer::getAtomicCustomDataFields();
			...
			$this->postSave($con);
			ScheduleResourcePeer::addInstanceToPool($this);
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = ScheduleResourcePeer::doInsert($this, $con);
					...
					$affectedObjects = ScheduleResourcePeer::doUpdate($this, $con);
					if($affectedObjects)
						$this->objectSaved = true;
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = ScheduleResourcePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = ScheduleResourcePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = ScheduleResourcePeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ScheduleResourcePeer::ID)) $criteria->add(ScheduleResourcePeer::ID, $this->id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(ScheduleResourcePeer::ID, $this->id);
		...
		if($this->alreadyInSave)
		{
			...
			if (count($this->modifiedColumns) == 2 && $this->isColumnModified(ScheduleResourcePeer::UPDATED_AT))
			{
				$theModifiedColumn = null;
				foreach($this->modifiedColumns as $modifiedColumn)
					if($modifiedColumn != ScheduleResourcePeer::UPDATED_AT)
						$theModifiedColumn = $modifiedColumn;
						
				$atomicColumns = ScheduleResourcePeer::getAtomicColumns();
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of ScheduleResource (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     ScheduleResource Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @var     ScheduleResource Clone of current object.
	 */
	 ...
	 * Stores the source object that this object copied from 
	 *
	 * @param      ScheduleResource $copiedFrom Clone of current object.
	 */
	public function setCopiedFrom(ScheduleResource $copiedFrom)
	{
	 ...
	 *
	 * @return     ScheduleResourcePeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new ScheduleResourcePeer();
		}
		return self::$peer;
	}
