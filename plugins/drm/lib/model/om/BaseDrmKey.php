...
/**
 * Base class that represents a row from the 'drm_key' table.
 *
 * 
 *
 * @package plugins.drm
 * @subpackage model.om
 */
abstract class BaseDrmKey extends BaseObject  implements Persistent {
	...
	{
		if(!isset($this->oldColumnsValues[DrmKeyPeer::ID]))
			$this->oldColumnsValues[DrmKeyPeer::ID] = $this->id;
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = DrmKeyPeer::ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[DrmKeyPeer::PARTNER_ID]))
			$this->oldColumnsValues[DrmKeyPeer::PARTNER_ID] = $this->partner_id;
		...
		if ($this->partner_id !== $v) {
			$this->partner_id = $v;
			$this->modifiedColumns[] = DrmKeyPeer::PARTNER_ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[DrmKeyPeer::PROVIDER]))
			$this->oldColumnsValues[DrmKeyPeer::PROVIDER] = $this->provider;
		...
		if ($this->provider !== $v) {
			$this->provider = $v;
			$this->modifiedColumns[] = DrmKeyPeer::PROVIDER;
		}
	...
	{
		if(!isset($this->oldColumnsValues[DrmKeyPeer::OBJECT_ID]))
			$this->oldColumnsValues[DrmKeyPeer::OBJECT_ID] = $this->object_id;
		...
		if ($this->object_id !== $v) {
			$this->object_id = $v;
			$this->modifiedColumns[] = DrmKeyPeer::OBJECT_ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[DrmKeyPeer::OBJECT_TYPE]))
			$this->oldColumnsValues[DrmKeyPeer::OBJECT_TYPE] = $this->object_type;
		...
		if ($this->object_type !== $v) {
			$this->object_type = $v;
			$this->modifiedColumns[] = DrmKeyPeer::OBJECT_TYPE;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = DrmKeyPeer::CREATED_AT;
			}
	...
	public function setUpdatedAt($v)
	{
		...
		if ( $this->updated_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = DrmKeyPeer::UPDATED_AT;
			}
		} // if either are not null
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				DrmKeyPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = DrmKeyPeer::doInsert($this, $con);
					...
					$affectedObjects = DrmKeyPeer::doUpdate($this, $con);
					if($affectedObjects)
						$this->objectSaved = true;
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = DrmKeyPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = DrmKeyPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = DrmKeyPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(DrmKeyPeer::ID)) $criteria->add(DrmKeyPeer::ID, $this->id);
		if ($this->isColumnModified(DrmKeyPeer::PARTNER_ID)) $criteria->add(DrmKeyPeer::PARTNER_ID, $this->partner_id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(DrmKeyPeer::ID, $this->id);
		
		if($this->alreadyInSave)
		{
			if (count($this->modifiedColumns) == 2 && $this->isColumnModified(DrmKeyPeer::UPDATED_AT))
			{
				$theModifiedColumn = null;
				foreach($this->modifiedColumns as $modifiedColumn)
					if($modifiedColumn != DrmKeyPeer::UPDATED_AT)
						$theModifiedColumn = $modifiedColumn;
						
				$atomicColumns = DrmKeyPeer::getAtomicColumns();
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of DrmKey (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     DrmKey Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @var     DrmKey Clone of current object.
	 */
	 ...
	 * Stores the source object that this object copied from 
	 *
	 * @param      DrmKey $copiedFrom Clone of current object.
	 */
	public function setCopiedFrom(DrmKey $copiedFrom)
	{
