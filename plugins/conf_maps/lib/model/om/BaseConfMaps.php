 ...
 * @subpackage model.om
 */
abstract class BaseConfMaps extends BaseObject  implements Persistent {
	...
	{
		if(!isset($this->oldColumnsValues[ConfMapsPeer::ID]))
			$this->oldColumnsValues[ConfMapsPeer::ID] = $this->id;
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = ConfMapsPeer::ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ConfMapsPeer::MAP_NAME]))
			$this->oldColumnsValues[ConfMapsPeer::MAP_NAME] = $this->map_name;
		...
		if ($this->map_name !== $v) {
			$this->map_name = $v;
			$this->modifiedColumns[] = ConfMapsPeer::MAP_NAME;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ConfMapsPeer::HOST_NAME]))
			$this->oldColumnsValues[ConfMapsPeer::HOST_NAME] = $this->host_name;
		...
		if ($this->host_name !== $v) {
			$this->host_name = $v;
			$this->modifiedColumns[] = ConfMapsPeer::HOST_NAME;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ConfMapsPeer::STATUS]))
			$this->oldColumnsValues[ConfMapsPeer::STATUS] = $this->status;
		...
		if ($this->status !== $v) {
			$this->status = $v;
			$this->modifiedColumns[] = ConfMapsPeer::STATUS;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ConfMapsPeer::REMARKS]))
			$this->oldColumnsValues[ConfMapsPeer::REMARKS] = $this->remarks;
		...
		if ($this->remarks !== $v) {
			$this->remarks = $v;
			$this->modifiedColumns[] = ConfMapsPeer::REMARKS;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ConfMapsPeer::CONTENT]))
			$this->oldColumnsValues[ConfMapsPeer::CONTENT] = $this->content;
		...
		if ($this->content !== $v) {
			$this->content = $v;
			$this->modifiedColumns[] = ConfMapsPeer::CONTENT;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->map_name = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->host_name = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		ConfMapsPeer::setUseCriteriaFilter(false);
		$criteria = $this->buildPkeyCriteria();
		ConfMapsPeer::addSelectColumns($criteria);
		$stmt = BasePeer::doSelect($criteria, $con);
		ConfMapsPeer::setUseCriteriaFilter(true);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				ConfMapsPeer::doDelete($this, $con);
				$this->postDelete($con);
	...
	protected function doSave(PropelPDO $con, $skipReload = false)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = ConfMapsPeer::doInsert($this, $con);
					if (!$skipReload) {
						$reloadObject = true;
					}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = ConfMapsPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = ConfMapsPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = ConfMapsPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setMapName($arr[$keys[1]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ConfMapsPeer::ID)) $criteria->add(ConfMapsPeer::ID, $this->id);
		if ($this->isColumnModified(ConfMapsPeer::MAP_NAME)) $criteria->add(ConfMapsPeer::MAP_NAME, $this->map_name);
		if ($this->isColumnModified(ConfMapsPeer::HOST_NAME)) $criteria->add(ConfMapsPeer::HOST_NAME, $this->host_name);
		if ($this->isColumnModified(ConfMapsPeer::STATUS)) $criteria->add(ConfMapsPeer::STATUS, $this->status);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(ConfMapsPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of ConfMaps (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     ConfMaps Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 */
	public function setCopiedFrom(ConfMaps $copiedFrom)
	{
