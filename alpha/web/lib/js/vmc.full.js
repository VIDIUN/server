...
/* vmc and vmc.vars defined in script block in vmc2success.php */
	...
	vmc.functions = {
		...
		closeEditor : function(saved) { // VSE
//			alert("saved="+saved);
			if(saved==0) {
		...
		openVcw : function(vs, conversion_profile) {
			...
			var params = {
				...
				quality: "high",
				wmode: "opaque" ,
				movie: vmc.vars.service_url + "/vcw/ui_conf_id/48613"
			};
...
//			}
		...
		openSupport : function(href) {
			...
			modal.innerHTML = '<div id="modal"><div id="titlebar"><a id="close" href="#close" onclick="vidiunCloseModalBox(); return false;"></a>' +
...
//				wmode				: "opaque"
		...
		content : {
			...
			flashvars : {
				moderationVDPVersion : "v3.1.6",
				drillDownVDPVersion  : "v3.1.6",
		...
		appstudio : {
			...
			flashvars : {
				...
				vdpUrl				: vmc.vars.flash_dir + "/vdp3/v3.1.6/vdp3.swf",
...
//				widget_id			: "_" + vmc.vars.partner_id
		...
		reports : {
			...
			flashvars : {
				drillDownVdpVersion	: "v3.1.6",
				drillDownVdpUiconf	: 48506,
				serverPath			: vmc.vars.service_url
			}
		}
	}
	vmc.utils.mergeJson(vmc.modules.appstudio.flashvars,{ "playlistAPI.vpl0Url"	: vmc.modules.appstudio.playlist_url + "_VMCSPL1", "playlistAPI.vpl1Url" : vmc.modules.appstudio.playlist_url + "_VMCSPL2" });
...
//			alert("doPreviewEmbed: id="+id+", name="+name+", description="+description+", is_playlist="+is_playlist+", uiconf_id="+uiconf_id);
			...
			if(id != "multitab_playlist") {
				...
				description = vmc.utils.escapeQuotes(description); // @todo: move to jw
...
//			console.log("uiconf_details="+uiconf_details);
			if(vmc.vars.jw) {
				...
				embed_code = vmc.preview_embed.buildVidiunEmbed(id, name, is_playlist, uiconf_id);
				preview_player = embed_code.replace('{FLAVOR}','vs=' + vmc.vars.vs + '&');
				embed_code = embed_code.replace('{FLAVOR}','');
			}
			var modal_html = '<div id="modal"><div id="titlebar"><a id="close" href="#close" onclick="vidiunCloseModalBox(); return false;"></a>' +
							 ...
							 (vmc.vars.jw ? jw_nomix_box_html : '') + '<div id="player_wrap">' + preview_player + '</div>' +
							 (vmc.vars.jw ? jw_options_html : vmc.preview_embed.buildRtmpOptions()) +
							 '<div class="label">Embed Code:</div> <textarea id="embed_code" rows="5" cols="" onclick="vmc.utils.copyCode();"' +
							 'readonly="true" style="width:' + (parseInt(uiconf_details.width)-10) + 'px;">' + embed_code + '</textarea>' +
							 '<div id="copy_msg">Press Ctrl+C to copy embed code (Command+C on Mac)</div><button id="select_code" onclick="vmc.utils.copyCode();">' +
...
//			var flavor_asset_name = vmc.utils.escapeQuotes(flavor_details.flavor_name) || "unknown";
			...
			$("#mbContent").addClass("new");
			var player_code = vmc.preview_embed.buildVidiunEmbed(entry_id,entry_name,false,vmc.vars.default_vdp);
...
//			$("#mbContent").addClass("new");
		...
		embed_code_template :	{
			object_tag :	'<object id="vidiun_player" name="vidiun_player" type="application/x-shockwave-flash" allowFullScreen="true" allowNetworking="all" ' +
							'allowScriptAccess="always" height="{HEIGHT}" width="{WIDTH}" data="http://{HOST}/index.php/vwidget/cache_st/' +
							...
							'value="#000000" /><param name="flashVars" value="{FLASHVARS}&{FLAVOR}" /><param name="movie" value="http://{HOST}/index.php/vwidget/cache_st/' +
							'{CACHE_ST}/wid/_{PARTNER_ID}/uiconf_id/{UICONF_ID}{ENTRY_ID}" /><a href="http://corp.vidiun.com">' +
							'video platform</a> <a href="http://corp.vidiun.com/technology/video_management">video management</a> <a href="' +
							'http://corp.vidiun.com/solutions/overview">video solutions</a> <a href="http://corp.vidiun.com/technology/' +
							'video_player">video player</a></object>',
			playlist_flashvars :	'playlistAPI.autoContinue=true&playlistAPI.autoInsert=true&playlistAPI.vpl0Name={PL_NAME}' +
...
layoutId=playlistLight&
	playlistAPI.autoContinue = v_pl_autoContinue
	playlistAPI.autoInsert = v_pl_autoInsertMedia
	playlistAPI.vpl0Name = v_pl_0_name
	v_pl_0_url = playlistAPI.vpl0Url
*/
		...
		buildVidiunEmbed : function(id, name, is_playlist, uiconf) {
...
//			alert("cache_st = " + cache_st);
			...
			if(!vmc.vars.jw) {
				...
				if(vmc.vars.embed_code_delivery_type == "rtmp") {
					embed_code = embed_code.replace("{FLASHVARS}", "streamerType=rtmp&amp;streamerUrl=rtmp://rtmpakmi.vidiun.com/ondemand&amp;rtmpFlavors=1&{FLASHVARS}");
				}
			}
