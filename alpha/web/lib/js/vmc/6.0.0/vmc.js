...
(function(vmc) {
	...
	Preview.openPreviewEmbed = function(options, previewService) {
		...
		var modalHeight = $(window).height() - 173;
	...
	Preview.getEmbedFlashVars = function(previewService, addVs) {
		...
		if(playlistId) {
			...
			if(vmc.functions.versionIsAtLeast(vmc.vars.min_vdp_version_for_playlist_api_v3, player.swf_version) 
...
vmcApp.controller('PreviewCtrl', ['$scope', 'previewService', function($scope, previewService) {
	...
	var setDeliveryTypes = function(player) {
			...
			var defaultType = $scope.deliveryType;
	...
	$scope.player = null;
	$scope.deliveryTypes = [];
	$scope.deliveryType = Preview.getDefault('deliveryType');
	$scope.embedTypes = [];
...
vmc.vars.api_host = vmc.vars.host + vmc.vars.port;
