...
class myNewBatchPartnerUsage extends myBatchBase
{
	...
	{
		VidiunLog::debug("calculating storage agg. for date $date");
		// set start points
		...
		while($partners_exists)
		{
			...
			if (!$partners)
			{
				VidiunLog::debug( "No more partners. offset: $start_pos , limit: $bulk_size ." );
				// set flag to exit while loop
				$partners_exists = false;
			} 
			else
			{
				VidiunLog::debug( "Looping ". ($start_pos + $bulk_size -1) ." partners, offset: $start_pos ." );
				...
				foreach($partners as $partner)
				{
					...
					VidiunLog::debug('Partner '.$partner_id.' => has total hosting of '.$total_hosting.' as for ['.$date.']');
					...
					try{
						...
						//$result = '';
						//var_dump($resultset);
						VidiunLog::debug("updated/added row for partner $partner_id, with amount1 $total_hosting ");
					}
					catch(Exception $ex)
					{
		...
		if(count($errors))
		{
			VidiunLog::debug("errors occurred: ".print_r($errors,true));
		}
	}
	...
	public function doMonthlyAggregation( $date )
	{
		...
		VidiunLog::debug("calculating monthly agg. for date $currentDate");
		...
		while($partners_exists)
		{
			...
			{		
				...
				foreach($partners as $partner)
				{
					...
					VidiunLog::debug("testing... not skiping partner ".$partner->getId());
					*/
					...
					{
						VidiunLog::debug( "loaded more than one monthly aggregation row for partner. something went wrong. partner ".$partner->getID() );
					}
	...
	private function getStorageAggregationFor( $partnerId, $date )
	{
		...
		//VidiunLog::debug("$partnerId and $date resulted in ".count($activity). " rows");
	...
	private function getTrafficFor( $partnerId, $date )
	{
		...
		//VidiunLog::debug("traffic ! $partnerId and $date resulted in ".count($activity). " rows");
		$_traffic = 0;
		if (count($activity) == 2)
		{
			$_traffic = $activity[0]->getAmount();
			//VidiunLog::debug("DB value (act[0]) = ".$activity[0]->getAmount().' my value = '.$_traffic);
			$_traffic += $activity[1]->getAmount();
			//VidiunLog::debug("DB value (act[1]) = ".$activity[1]->getAmount().' my value = '.$_traffic);
		}
		elseif (count($activity) == 1)
		{
			$_traffic = $activity[0]->getAmount();
			//VidiunLog::debug("DB value (act[0] only) = ".$activity[0]->getAmount().' my value = '.$_traffic);
		}
