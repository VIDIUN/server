...
class myFileIndicator
{
	...
	public function __construct ( $file_name )
	{
		if ( ! self::$s_indicator_path  )
		{
			...
			vFile::fullMkdir( self::$s_indicator_path . "dummy.txt" );			
		}
	...
	{
		$indicator_list = vFile::recursiveDirList( self::$s_indicator_path ,true , false , $this->pattern );
		return ( count ( $indicator_list ) > 0 );
	}
	
	public function addIndicator ( $suffix = null )
	{
		$indicator_list = vFile::recursiveDirList( self::$s_indicator_path ,true , false , $this->pattern , 0 , 5 );
	...
	public function removeIndicator ( )
	{
		...
		{
			vFile::deleteFile( $indicator_list[0] );
		}
		if ( count ( $indicator_list ) > 100 )
		{
			...
			for ( $i = 1 ; $i < 50 ; $i++)
			{
				vFile::deleteFile( $indicator_list[$i] );
			}
		}
