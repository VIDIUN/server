...
abstract class exportableAsset extends asset
{
	...
	public function setStatusLocalReady()
	{
		...
		foreach($externalStorages as $externalStorage)
		{
			...
			{
				VidiunLog::info('Asset id ['.$this->getId().'] is required to export to profile ['.$externalStorage->getId().'] - setting status to [EXPORTING]');
				$newStatus = asset::ASSET_STATUS_EXPORTING;
				break;
			}
		}
		VidiunLog::info('Setting status to ['.$newStatus.']');
	...
	private function requiredToExportAsset(StorageProfile $storage)
	{
		...
		if (!$storage->triggerFitsReadyAsset($this->getEntryId())) {
			VidiunLog::info('Asset id ['.$this->getId().'] is not ready to export to profile ['.$storage->getId().']');
			return false;
		}
		...
		{
			VidiunLog::info('Should not export asset id ['.$this->getId().'] to profile ['.$storage->getId().']');
			return false;
		}
		...
		foreach ($keys as $key)
		{
			...
			{
				VidiunLog::info('Asset id ['.$this->getId().'] is currently being exported to profile ['.$storage->getId().']');
				return true;
			}
		}
		return false;
