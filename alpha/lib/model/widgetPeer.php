...
{
	const WIDGET_PEER_JOIN_VSHOW = 1;
...
*/
	...
	public static function retrieveByPK($pk, PropelPDO $con = null, $join = null)
	{
		...
		// for now supporting only vshow,entry,vshow+entry and all 
		if ( $join == self::WIDGET_PEER_JOIN_VSHOW )
			$res = self::doSelectJoinvshow( $c , $con );
		elseif ( $join == self::WIDGET_PEER_JOIN_ENTRY )
			$res = self::doSelectJoinentry( $c , $con );
		elseif ( $join == self::WIDGET_PEER_JOIN_ENTRY + self::WIDGET_PEER_JOIN_VSHOW )
