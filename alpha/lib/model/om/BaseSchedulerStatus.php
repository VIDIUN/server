...
/**
 * Base class that represents a row from the 'scheduler_status' table.
 *
 * 
 *
 * @package Core
...
abstract class BaseSchedulerStatus extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the worker_type field.
	 * @var        int
	 */
	...
	/**
	 * Get the [worker_type] column value.
	 * 
	 * @return     int
	 */
	 ...
	 * @return     SchedulerStatus The current object (for fluent API support)
	 */
	...
	/**
	 * Set the value of [worker_type] column.
	 * 
	 * @param      int $v new value
	 * @return     SchedulerStatus The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->worker_id = ($row[$startcol + 7] !== null) ? (int) $row[$startcol + 7] : null;
			$this->worker_configured_id = ($row[$startcol + 8] !== null) ? (int) $row[$startcol + 8] : null;
			$this->worker_type = ($row[$startcol + 9] !== null) ? (int) $row[$startcol + 9] : null;
			...
			return $startcol + 12; // 12 = SchedulerStatusPeer::NUM_COLUMNS - SchedulerStatusPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		SchedulerStatusPeer::setUseCriteriaFilter(false);
		$stmt = SchedulerStatusPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
		SchedulerStatusPeer::setUseCriteriaFilter(true);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				SchedulerStatusPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$affectedObjects = SchedulerStatusPeer::doUpdate($this, $con);
					if($affectedObjects)
						$this->objectSaved = true;
	...
	public function buildPkeyCriteria()
	{
		...
		if($this->alreadyInSave && count($this->modifiedColumns) == 2 && $this->isColumnModified(SchedulerStatusPeer::UPDATED_AT))
		{
			...
			foreach($this->modifiedColumns as $modifiedColumn)
				if($modifiedColumn != SchedulerStatusPeer::UPDATED_AT)
					$theModifiedColumn = $modifiedColumn;
					
			$atomicColumns = SchedulerStatusPeer::getAtomicColumns();
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of SchedulerStatus (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     SchedulerStatus Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @var     SchedulerStatus Clone of current object.
	 */
	 ...
	 * Stores the source object that this object copied from 
	 *
	 * @param      SchedulerStatus $copiedFrom Clone of current object.
	 */
	public function setCopiedFrom(SchedulerStatus $copiedFrom)
	{
