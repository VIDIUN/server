...
abstract class BaseScheduler extends BaseObject  implements Persistent {
	...
	/**
	 * Initializes internal state of BaseScheduler object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     Scheduler The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->configured_id = ($row[$startcol + 5] !== null) ? (int) $row[$startcol + 5] : null;
			$this->name = ($row[$startcol + 6] !== null) ? (string) $row[$startcol + 6] : null;
			$this->description = ($row[$startcol + 7] !== null) ? (string) $row[$startcol + 7] : null;
			$this->statuses = ($row[$startcol + 8] !== null) ? (string) $row[$startcol + 8] : null;
			$this->last_status = ($row[$startcol + 9] !== null) ? (string) $row[$startcol + 9] : null;
			$this->host = ($row[$startcol + 10] !== null) ? (string) $row[$startcol + 10] : null;
			$this->resetModified();
			...
			return $startcol + 11; // 11 = SchedulerPeer::NUM_COLUMNS - SchedulerPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				SchedulerPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$affectedObjects = SchedulerPeer::doUpdate($this, $con);
					if($affectedObjects)
						$this->objectSaved = true;
	...
	public function buildPkeyCriteria()
	{
		...
		if($this->alreadyInSave && count($this->modifiedColumns) == 2 && $this->isColumnModified(SchedulerPeer::UPDATED_AT))
		{
			$theModifiedColumn = null;
			foreach($this->modifiedColumns as $modifiedColumn)
				if($modifiedColumn != SchedulerPeer::UPDATED_AT)
					$theModifiedColumn = $modifiedColumn;
					
			$atomicColumns = SchedulerPeer::getAtomicColumns();
	 ...
	 *
	 * @var     Scheduler Clone of current object.
	 */
	 ...
	 * Stores the source object that this object copied from 
	 *
	 * @param      Scheduler $copiedFrom Clone of current object.
	 */
	public function setCopiedFrom(Scheduler $copiedFrom)
	{
