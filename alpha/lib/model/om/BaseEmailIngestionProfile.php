		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$affectedObjects = EmailIngestionProfilePeer::doUpdate($this, $con);
					if($affectedObjects)
						$this->objectSaved = true;
	...
	public function buildPkeyCriteria()
	{
		...
		if($this->alreadyInSave && count($this->modifiedColumns) == 2 && $this->isColumnModified(EmailIngestionProfilePeer::UPDATED_AT))
		{
			...
			foreach($this->modifiedColumns as $modifiedColumn)
				if($modifiedColumn != EmailIngestionProfilePeer::UPDATED_AT)
					$theModifiedColumn = $modifiedColumn;
					
			$atomicColumns = EmailIngestionProfilePeer::getAtomicColumns();
