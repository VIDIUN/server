...
abstract class BasecategoryEntry extends BaseObject  implements Persistent {
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		...
		try {
			...
			return $startcol + 11; // 11 = categoryEntryPeer::NUM_COLUMNS - categoryEntryPeer::NUM_LAZY_LOAD_COLUMNS).
		...
		try {
			...
			for ($retries = 1; $retries < VidiunPDO::SAVE_MAX_RETRIES; $retries++)
			{
               $affectedRows = $this->doSave($con);
				...
				$validUpdate = true;
				$atomicCustomDataFields = categoryEntryPeer::getAtomicCustomDataFields();
