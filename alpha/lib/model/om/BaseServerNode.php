...
/**
 * Base class that represents a row from the 'server_node' table.
 *
 * 
 *
 * @package Core
 * @subpackage model.om
 */
abstract class BaseServerNode extends BaseObject  implements Persistent {
	...
	/**
	 * Initializes internal state of BaseServerNode object.
	 * @see        applyDefaults()
	 */
	...
	{
		if(!isset($this->oldColumnsValues[ServerNodePeer::ID]))
			$this->oldColumnsValues[ServerNodePeer::ID] = $this->id;
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = ServerNodePeer::ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ServerNodePeer::DC]))
			$this->oldColumnsValues[ServerNodePeer::DC] = $this->dc;
		...
		if ($this->dc !== $v) {
			$this->dc = $v;
			$this->modifiedColumns[] = ServerNodePeer::DC;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = ServerNodePeer::CREATED_AT;
			}
	...
	public function setUpdatedAt($v)
	{
		...
		if ( $this->updated_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = ServerNodePeer::UPDATED_AT;
			}
		} // if either are not null
	...
	{
		if(!isset($this->oldColumnsValues[ServerNodePeer::HEARTBEAT_TIME]))
			$this->oldColumnsValues[ServerNodePeer::HEARTBEAT_TIME] = $this->heartbeat_time;
		...
		if ( $this->heartbeat_time !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = ServerNodePeer::HEARTBEAT_TIME;
			}
		} // if either are not null
	...
	{
		if(!isset($this->oldColumnsValues[ServerNodePeer::PARTNER_ID]))
			$this->oldColumnsValues[ServerNodePeer::PARTNER_ID] = $this->partner_id;
		...
		if ($this->partner_id !== $v) {
			$this->partner_id = $v;
			$this->modifiedColumns[] = ServerNodePeer::PARTNER_ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ServerNodePeer::NAME]))
			$this->oldColumnsValues[ServerNodePeer::NAME] = $this->name;
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = ServerNodePeer::NAME;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ServerNodePeer::SYSTEM_NAME]))
			$this->oldColumnsValues[ServerNodePeer::SYSTEM_NAME] = $this->system_name;
		...
		if ($this->system_name !== $v) {
			$this->system_name = $v;
			$this->modifiedColumns[] = ServerNodePeer::SYSTEM_NAME;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ServerNodePeer::DESCRIPTION]))
			$this->oldColumnsValues[ServerNodePeer::DESCRIPTION] = $this->description;
		...
		if ($this->description !== $v) {
			$this->description = $v;
			$this->modifiedColumns[] = ServerNodePeer::DESCRIPTION;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ServerNodePeer::STATUS]))
			$this->oldColumnsValues[ServerNodePeer::STATUS] = $this->status;
		...
		if ($this->status !== $v) {
			$this->status = $v;
			$this->modifiedColumns[] = ServerNodePeer::STATUS;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ServerNodePeer::TYPE]))
			$this->oldColumnsValues[ServerNodePeer::TYPE] = $this->type;
		...
		if ($this->type !== $v || $this->isNew()) {
			$this->type = $v;
			$this->modifiedColumns[] = ServerNodePeer::TYPE;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ServerNodePeer::TAGS]))
			$this->oldColumnsValues[ServerNodePeer::TAGS] = $this->tags;
		...
		if ($this->tags !== $v) {
			$this->tags = $v;
			$this->modifiedColumns[] = ServerNodePeer::TAGS;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ServerNodePeer::HOST_NAME]))
			$this->oldColumnsValues[ServerNodePeer::HOST_NAME] = $this->host_name;
		...
		if ($this->host_name !== $v) {
			$this->host_name = $v;
			$this->modifiedColumns[] = ServerNodePeer::HOST_NAME;
		}
	...
	{
		if(!isset($this->oldColumnsValues[ServerNodePeer::PLAYBACK_HOST_NAME]))
			$this->oldColumnsValues[ServerNodePeer::PLAYBACK_HOST_NAME] = $this->playback_host_name;
		...
		if ($this->playback_host_name !== $v) {
			...
			$this->modifiedColumns[] = ServerNodePeer::PLAYBACK_HOST_NAME;
		}
	...
	public function setCustomData($v)
	{
		...
		if ($this->custom_data !== $v) {
			$this->custom_data = $v;
			$this->modifiedColumns[] = ServerNodePeer::CUSTOM_DATA;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		...
		try {
			...
			$this->dc = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		ServerNodePeer::setUseCriteriaFilter(false);
		$criteria = $this->buildPkeyCriteria();
		ServerNodePeer::addSelectColumns($criteria);
		$stmt = BasePeer::doSelect($criteria, $con);
		ServerNodePeer::setUseCriteriaFilter(true);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				ServerNodePeer::doDelete($this, $con);
				$this->postDelete($con);
		...
		try {
			...
			for ($retries = 1; $retries < VidiunPDO::SAVE_MAX_RETRIES; $retries++)
			{
               $affectedRows = $this->doSave($con);
                if ($affectedRows || !$this->isColumnModified(ServerNodePeer::CUSTOM_DATA)) //ask if custom_data wasn't modified to avoid retry with atomic column 
                	break;
				...
				$validUpdate = true;
				$atomicCustomDataFields = ServerNodePeer::getAtomicCustomDataFields();
			...
			$this->postSave($con);
			ServerNodePeer::addInstanceToPool($this);
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = ServerNodePeer::doInsert($this, $con);
					...
					$affectedObjects = ServerNodePeer::doUpdate($this, $con);
					if($affectedObjects)
						$this->objectSaved = true;
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = ServerNodePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = ServerNodePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = ServerNodePeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getDc(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setDc($arr[$keys[1]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ServerNodePeer::ID)) $criteria->add(ServerNodePeer::ID, $this->id);
		if ($this->isColumnModified(ServerNodePeer::DC)) $criteria->add(ServerNodePeer::DC, $this->dc);
		if ($this->isColumnModified(ServerNodePeer::CREATED_AT)) $criteria->add(ServerNodePeer::CREATED_AT, $this->created_at);
		if ($this->isColumnModified(ServerNodePeer::UPDATED_AT)) $criteria->add(ServerNodePeer::UPDATED_AT, $this->updated_at);
		if ($this->isColumnModified(ServerNodePeer::HEARTBEAT_TIME)) $criteria->add(ServerNodePeer::HEARTBEAT_TIME, $this->heartbeat_time);
		if ($this->isColumnModified(ServerNodePeer::PARTNER_ID)) $criteria->add(ServerNodePeer::PARTNER_ID, $this->partner_id);
		if ($this->isColumnModified(ServerNodePeer::NAME)) $criteria->add(ServerNodePeer::NAME, $this->name);
		if ($this->isColumnModified(ServerNodePeer::SYSTEM_NAME)) $criteria->add(ServerNodePeer::SYSTEM_NAME, $this->system_name);
		if ($this->isColumnModified(ServerNodePeer::DESCRIPTION)) $criteria->add(ServerNodePeer::DESCRIPTION, $this->description);
		if ($this->isColumnModified(ServerNodePeer::STATUS)) $criteria->add(ServerNodePeer::STATUS, $this->status);
		if ($this->isColumnModified(ServerNodePeer::TYPE)) $criteria->add(ServerNodePeer::TYPE, $this->type);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(ServerNodePeer::ID, $this->id);
		
		if($this->alreadyInSave)
		{
			if ($this->isColumnModified(ServerNodePeer::CUSTOM_DATA))
			{
				if (!is_null($this->custom_data_md5))
					$criteria->add(ServerNodePeer::CUSTOM_DATA, "MD5(cast(" . ServerNodePeer::CUSTOM_DATA . " as char character set latin1)) = '$this->custom_data_md5'", Criteria::CUSTOM);
					//casting to latin char set to avoid mysql and php md5 difference
				else 
					$criteria->add(ServerNodePeer::CUSTOM_DATA, NULL, Criteria::ISNULL);
			}
			
			if (count($this->modifiedColumns) == 2 && $this->isColumnModified(ServerNodePeer::UPDATED_AT))
			{
				$theModifiedColumn = null;
				foreach($this->modifiedColumns as $modifiedColumn)
					if($modifiedColumn != ServerNodePeer::UPDATED_AT)
						$theModifiedColumn = $modifiedColumn;
						
				$atomicColumns = ServerNodePeer::getAtomicColumns();
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of ServerNode (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     ServerNode Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @var     ServerNode Clone of current object.
	 */
	 ...
	 * Stores the source object that this object copied from 
	 *
	 * @param      ServerNode $copiedFrom Clone of current object.
	 */
	public function setCopiedFrom(ServerNode $copiedFrom)
	{
	 ...
	 *
	 * @return     ServerNodePeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new ServerNodePeer();
		}
		return self::$peer;
	}
