...
abstract class BaseUserLoginData extends BaseObject  implements Persistent {
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$affectedObjects = UserLoginDataPeer::doUpdate($this, $con);
					if($affectedObjects)
						$this->objectSaved = true;
	...
	public function buildPkeyCriteria()
	{
		...
		if($this->alreadyInSave && count($this->modifiedColumns) == 2 && $this->isColumnModified(UserLoginDataPeer::UPDATED_AT))
		{
			$theModifiedColumn = null;
			foreach($this->modifiedColumns as $modifiedColumn)
				if($modifiedColumn != UserLoginDataPeer::UPDATED_AT)
					$theModifiedColumn = $modifiedColumn;
					
			$atomicColumns = UserLoginDataPeer::getAtomicColumns();
