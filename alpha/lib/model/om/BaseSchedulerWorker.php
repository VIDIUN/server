...
/**
 * Base class that represents a row from the 'scheduler_worker' table.
 *
 * 
 *
 * @package Core
...
abstract class BaseSchedulerWorker extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the configured_id field.
	 * @var        int
	 */
	...
	/**
	 * The value for the statuses field.
	 * Note: this column has a database default value of: ''
	 * @var        string
	 */
	protected $statuses;
	...
	/**
	 * The value for the last_status field.
	 * @var        string
	 */
	...
	/**
	 * Initializes internal state of BaseSchedulerWorker object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [configured_id] column value.
	 * 
	 * @return     int
	 */
	...
	/**
	 * Get the [optionally formatted] temporal [last_status] column value.
	 * 
	 * This accessor only only work with unix epoch dates.  Consider enabling the propel.useDateTimeClass
	 * option in order to avoid converstions to integers (which are limited in the dates they can express).
	 *
	...
	public function getLastStatus($format = 'Y-m-d H:i:s')
	{
		...
		if ($this->last_status === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->last_status);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->last_status, true), $x);
			}
		}
	 ...
	 * @return     SchedulerWorker The current object (for fluent API support)
	 */
	public function setId($v)
	{
		if(!isset($this->oldColumnsValues[SchedulerWorkerPeer::ID]))
			$this->oldColumnsValues[SchedulerWorkerPeer::ID] = $this->id;
	...
	public function setCreatedBy($v)
	{
		if(!isset($this->oldColumnsValues[SchedulerWorkerPeer::CREATED_BY]))
			$this->oldColumnsValues[SchedulerWorkerPeer::CREATED_BY] = $this->created_by;
	...
	public function setUpdatedBy($v)
	{
		if(!isset($this->oldColumnsValues[SchedulerWorkerPeer::UPDATED_BY]))
			$this->oldColumnsValues[SchedulerWorkerPeer::UPDATED_BY] = $this->updated_by;
	...
	public function setSchedulerId($v)
	{
		if(!isset($this->oldColumnsValues[SchedulerWorkerPeer::SCHEDULER_ID]))
			$this->oldColumnsValues[SchedulerWorkerPeer::SCHEDULER_ID] = $this->scheduler_id;
	...
	public function setSchedulerConfiguredId($v)
	{
		if(!isset($this->oldColumnsValues[SchedulerWorkerPeer::SCHEDULER_CONFIGURED_ID]))
			$this->oldColumnsValues[SchedulerWorkerPeer::SCHEDULER_CONFIGURED_ID] = $this->scheduler_configured_id;
	...
	/**
	 * Set the value of [configured_id] column.
	 * 
	 * @param      int $v new value
	 * @return     SchedulerWorker The current object (for fluent API support)
	 */
	public function setConfiguredId($v)
	{
		if(!isset($this->oldColumnsValues[SchedulerWorkerPeer::CONFIGURED_ID]))
			$this->oldColumnsValues[SchedulerWorkerPeer::CONFIGURED_ID] = $this->configured_id;
	...
	public function setType($v)
	{
		if(!isset($this->oldColumnsValues[SchedulerWorkerPeer::TYPE]))
			$this->oldColumnsValues[SchedulerWorkerPeer::TYPE] = $this->type;
	...
	public function setName($v)
	{
		if(!isset($this->oldColumnsValues[SchedulerWorkerPeer::NAME]))
			$this->oldColumnsValues[SchedulerWorkerPeer::NAME] = $this->name;
	...
	public function setDescription($v)
	{
		if(!isset($this->oldColumnsValues[SchedulerWorkerPeer::DESCRIPTION]))
			$this->oldColumnsValues[SchedulerWorkerPeer::DESCRIPTION] = $this->description;
	...
	/**
	 * Set the value of [statuses] column.
	 * 
	 * @param      string $v new value
	 * @return     SchedulerWorker The current object (for fluent API support)
	 */
	public function setStatuses($v)
	{
		if(!isset($this->oldColumnsValues[SchedulerWorkerPeer::STATUSES]))
			$this->oldColumnsValues[SchedulerWorkerPeer::STATUSES] = $this->statuses;

		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->statuses !== $v || $this->isNew()) {
			$this->statuses = $v;
	...
	/**
	 * Sets the value of [last_status] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.  Empty string will
	 *						be treated as NULL for temporal objects.
	 * @return     SchedulerWorker The current object (for fluent API support)
	...
	public function setLastStatus($v)
	{
		if(!isset($this->oldColumnsValues[SchedulerWorkerPeer::LAST_STATUS]))
			$this->oldColumnsValues[SchedulerWorkerPeer::LAST_STATUS] = $this->last_status;
		...
		if ( $this->last_status !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->last_status !== null && $tmpDt = new DateTime($this->last_status)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->last_status = ($dt ? $dt->format('Y-m-d H:i:s') : null);
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->scheduler_id = ($row[$startcol + 5] !== null) ? (int) $row[$startcol + 5] : null;
			$this->scheduler_configured_id = ($row[$startcol + 6] !== null) ? (int) $row[$startcol + 6] : null;
			$this->configured_id = ($row[$startcol + 7] !== null) ? (int) $row[$startcol + 7] : null;
			...
			$this->statuses = ($row[$startcol + 11] !== null) ? (string) $row[$startcol + 11] : null;
			$this->last_status = ($row[$startcol + 12] !== null) ? (string) $row[$startcol + 12] : null;
			$this->resetModified();
			...
			return $startcol + 13; // 13 = SchedulerWorkerPeer::NUM_COLUMNS - SchedulerWorkerPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		SchedulerWorkerPeer::setUseCriteriaFilter(false);
		$stmt = SchedulerWorkerPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
		SchedulerWorkerPeer::setUseCriteriaFilter(true);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				SchedulerWorkerPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$affectedObjects = SchedulerWorkerPeer::doUpdate($this, $con);
					if($affectedObjects)
						$this->objectSaved = true;
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of SchedulerWorker (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     SchedulerWorker Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @var     SchedulerWorker Clone of current object.
	 */
	 ...
	 * Stores the source object that this object copied from 
	 *
	 * @param      SchedulerWorker $copiedFrom Clone of current object.
	 */
	public function setCopiedFrom(SchedulerWorker $copiedFrom)
	{
