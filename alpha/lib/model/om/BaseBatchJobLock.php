...
/**
 * Base class that represents a row from the 'batch_job_lock' table.
 *
 * 
 *
 * @package Core
 * @subpackage model.om
 */
abstract class BaseBatchJobLock extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array BatchJob[] Collection to store aggregation of BatchJob objects.
	 */
	protected $collBatchJobs;

	/**
	 * @var        Criteria The criteria used to select the current contents of collBatchJobs.
	 */
	private $lastBatchJobCriteria = null;
	...
	/**
	 * Initializes internal state of BaseBatchJobLock object.
	 * @see        applyDefaults()
	 */
	...
	{
		if(!isset($this->oldColumnsValues[BatchJobLockPeer::ID]))
			$this->oldColumnsValues[BatchJobLockPeer::ID] = $this->id;
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = BatchJobLockPeer::ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[BatchJobLockPeer::JOB_TYPE]))
			$this->oldColumnsValues[BatchJobLockPeer::JOB_TYPE] = $this->job_type;
		...
		if ($this->job_type !== $v) {
			$this->job_type = $v;
			$this->modifiedColumns[] = BatchJobLockPeer::JOB_TYPE;
		}
	...
	{
		if(!isset($this->oldColumnsValues[BatchJobLockPeer::JOB_SUB_TYPE]))
			$this->oldColumnsValues[BatchJobLockPeer::JOB_SUB_TYPE] = $this->job_sub_type;
		...
		if ($this->job_sub_type !== $v) {
			$this->job_sub_type = $v;
			$this->modifiedColumns[] = BatchJobLockPeer::JOB_SUB_TYPE;
		}
	...
	{
		if(!isset($this->oldColumnsValues[BatchJobLockPeer::OBJECT_ID]))
			$this->oldColumnsValues[BatchJobLockPeer::OBJECT_ID] = $this->object_id;
		...
		if ($this->object_id !== $v || $this->isNew()) {
			$this->object_id = $v;
			$this->modifiedColumns[] = BatchJobLockPeer::OBJECT_ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[BatchJobLockPeer::OBJECT_TYPE]))
			$this->oldColumnsValues[BatchJobLockPeer::OBJECT_TYPE] = $this->object_type;
		...
		if ($this->object_type !== $v) {
			$this->object_type = $v;
			$this->modifiedColumns[] = BatchJobLockPeer::OBJECT_TYPE;
		}
	...
	{
		if(!isset($this->oldColumnsValues[BatchJobLockPeer::ESTIMATED_EFFORT]))
			$this->oldColumnsValues[BatchJobLockPeer::ESTIMATED_EFFORT] = $this->estimated_effort;
		...
		if ($this->estimated_effort !== $v) {
			...
			$this->modifiedColumns[] = BatchJobLockPeer::ESTIMATED_EFFORT;
		}
	...
	{
		if(!isset($this->oldColumnsValues[BatchJobLockPeer::STATUS]))
			$this->oldColumnsValues[BatchJobLockPeer::STATUS] = $this->status;
		...
		if ($this->status !== $v) {
			$this->status = $v;
			$this->modifiedColumns[] = BatchJobLockPeer::STATUS;
		}
	...
	{
		if(!isset($this->oldColumnsValues[BatchJobLockPeer::START_AT]))
			$this->oldColumnsValues[BatchJobLockPeer::START_AT] = $this->start_at;
		...
		if ( $this->start_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = BatchJobLockPeer::START_AT;
			}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = BatchJobLockPeer::CREATED_AT;
			}
		} // if either are not null
	...
	{
		if(!isset($this->oldColumnsValues[BatchJobLockPeer::PRIORITY]))
			$this->oldColumnsValues[BatchJobLockPeer::PRIORITY] = $this->priority;
		...
		if ($this->priority !== $v) {
			$this->priority = $v;
			$this->modifiedColumns[] = BatchJobLockPeer::PRIORITY;
		}
	...
	{
		if(!isset($this->oldColumnsValues[BatchJobLockPeer::URGENCY]))
			$this->oldColumnsValues[BatchJobLockPeer::URGENCY] = $this->urgency;
		...
		if ($this->urgency !== $v) {
			$this->urgency = $v;
			$this->modifiedColumns[] = BatchJobLockPeer::URGENCY;
		}
	...
	{
		if(!isset($this->oldColumnsValues[BatchJobLockPeer::ENTRY_ID]))
			$this->oldColumnsValues[BatchJobLockPeer::ENTRY_ID] = $this->entry_id;
		...
		if ($this->entry_id !== $v || $this->isNew()) {
			$this->entry_id = $v;
			$this->modifiedColumns[] = BatchJobLockPeer::ENTRY_ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[BatchJobLockPeer::PARTNER_ID]))
			$this->oldColumnsValues[BatchJobLockPeer::PARTNER_ID] = $this->partner_id;
		...
		if ($this->partner_id !== $v || $this->isNew()) {
			$this->partner_id = $v;
			$this->modifiedColumns[] = BatchJobLockPeer::PARTNER_ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[BatchJobLockPeer::SCHEDULER_ID]))
			$this->oldColumnsValues[BatchJobLockPeer::SCHEDULER_ID] = $this->scheduler_id;
		...
		if ($this->scheduler_id !== $v) {
			$this->scheduler_id = $v;
			$this->modifiedColumns[] = BatchJobLockPeer::SCHEDULER_ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[BatchJobLockPeer::WORKER_ID]))
			$this->oldColumnsValues[BatchJobLockPeer::WORKER_ID] = $this->worker_id;
		...
		if ($this->worker_id !== $v) {
			$this->worker_id = $v;
			$this->modifiedColumns[] = BatchJobLockPeer::WORKER_ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[BatchJobLockPeer::BATCH_INDEX]))
			$this->oldColumnsValues[BatchJobLockPeer::BATCH_INDEX] = $this->batch_index;
		...
		if ($this->batch_index !== $v) {
			$this->batch_index = $v;
			$this->modifiedColumns[] = BatchJobLockPeer::BATCH_INDEX;
		}
	...
	{
		if(!isset($this->oldColumnsValues[BatchJobLockPeer::EXPIRATION]))
			$this->oldColumnsValues[BatchJobLockPeer::EXPIRATION] = $this->expiration;
		...
		if ( $this->expiration !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = BatchJobLockPeer::EXPIRATION;
			}
		} // if either are not null
	...
	{
		if(!isset($this->oldColumnsValues[BatchJobLockPeer::EXECUTION_ATTEMPTS]))
			$this->oldColumnsValues[BatchJobLockPeer::EXECUTION_ATTEMPTS] = $this->execution_attempts;
		...
		if ($this->execution_attempts !== $v) {
			...
			$this->modifiedColumns[] = BatchJobLockPeer::EXECUTION_ATTEMPTS;
		}
	...
	{
		if(!isset($this->oldColumnsValues[BatchJobLockPeer::VERSION]))
			$this->oldColumnsValues[BatchJobLockPeer::VERSION] = $this->version;
		...
		if ($this->version !== $v) {
			$this->version = $v;
			$this->modifiedColumns[] = BatchJobLockPeer::VERSION;
		}
	...
	{
		if(!isset($this->oldColumnsValues[BatchJobLockPeer::DC]))
			$this->oldColumnsValues[BatchJobLockPeer::DC] = $this->dc;
		...
		if ($this->dc !== $v) {
			$this->dc = $v;
			$this->modifiedColumns[] = BatchJobLockPeer::DC;
		}
	...
	{
		if(!isset($this->oldColumnsValues[BatchJobLockPeer::BATCH_JOB_ID]))
			$this->oldColumnsValues[BatchJobLockPeer::BATCH_JOB_ID] = $this->batch_job_id;
		...
		if ($this->batch_job_id !== $v) {
			$this->batch_job_id = $v;
			$this->modifiedColumns[] = BatchJobLockPeer::BATCH_JOB_ID;
		}
	...
	public function setCustomData($v)
	{
		...
		if ($this->custom_data !== $v) {
			$this->custom_data = $v;
			$this->modifiedColumns[] = BatchJobLockPeer::CUSTOM_DATA;
		}
	...
	{
		if(!isset($this->oldColumnsValues[BatchJobLockPeer::BATCH_VERSION]))
			$this->oldColumnsValues[BatchJobLockPeer::BATCH_VERSION] = $this->batch_version;
		...
		if ($this->batch_version !== $v) {
			...
			$this->modifiedColumns[] = BatchJobLockPeer::BATCH_VERSION;
		}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		BatchJobLockPeer::setUseCriteriaFilter(false);
		$stmt = BatchJobLockPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
		BatchJobLockPeer::setUseCriteriaFilter(true);
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->aBatchJob = null;
			$this->collBatchJobs = null;
			$this->lastBatchJobCriteria = null;
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				BatchJobLockPeer::doDelete($this, $con);
				$this->postDelete($con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				BatchJobLockPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = BatchJobLockPeer::doInsert($this, $con);
					...
					$affectedObjects = BatchJobLockPeer::doUpdate($this, $con);
					if($affectedObjects)
						$this->objectSaved = true;
			...
			if ($this->collBatchJobs !== null) {
				foreach ($this->collBatchJobs as $referrerFK) {
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = BatchJobLockPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = BatchJobLockPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(BatchJobLockPeer::ID)) $criteria->add(BatchJobLockPeer::ID, $this->id);
		if ($this->isColumnModified(BatchJobLockPeer::JOB_TYPE)) $criteria->add(BatchJobLockPeer::JOB_TYPE, $this->job_type);
		if ($this->isColumnModified(BatchJobLockPeer::JOB_SUB_TYPE)) $criteria->add(BatchJobLockPeer::JOB_SUB_TYPE, $this->job_sub_type);
		if ($this->isColumnModified(BatchJobLockPeer::OBJECT_ID)) $criteria->add(BatchJobLockPeer::OBJECT_ID, $this->object_id);
		...
		if ($this->isColumnModified(BatchJobLockPeer::URGENCY)) $criteria->add(BatchJobLockPeer::URGENCY, $this->urgency);
		if ($this->isColumnModified(BatchJobLockPeer::ENTRY_ID)) $criteria->add(BatchJobLockPeer::ENTRY_ID, $this->entry_id);
		if ($this->isColumnModified(BatchJobLockPeer::PARTNER_ID)) $criteria->add(BatchJobLockPeer::PARTNER_ID, $this->partner_id);
		if ($this->isColumnModified(BatchJobLockPeer::SCHEDULER_ID)) $criteria->add(BatchJobLockPeer::SCHEDULER_ID, $this->scheduler_id);
		if ($this->isColumnModified(BatchJobLockPeer::WORKER_ID)) $criteria->add(BatchJobLockPeer::WORKER_ID, $this->worker_id);
		if ($this->isColumnModified(BatchJobLockPeer::BATCH_INDEX)) $criteria->add(BatchJobLockPeer::BATCH_INDEX, $this->batch_index);
		if ($this->isColumnModified(BatchJobLockPeer::EXPIRATION)) $criteria->add(BatchJobLockPeer::EXPIRATION, $this->expiration);
		if ($this->isColumnModified(BatchJobLockPeer::EXECUTION_ATTEMPTS)) $criteria->add(BatchJobLockPeer::EXECUTION_ATTEMPTS, $this->execution_attempts);
		if ($this->isColumnModified(BatchJobLockPeer::VERSION)) $criteria->add(BatchJobLockPeer::VERSION, $this->version);
		if ($this->isColumnModified(BatchJobLockPeer::DC)) $criteria->add(BatchJobLockPeer::DC, $this->dc);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(BatchJobLockPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of BatchJobLock (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getBatchJobs() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addBatchJob($relObj->copy($deepCopy));
				}
			}
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     BatchJobLock Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @var     BatchJobLock Clone of current object.
	 */
	 ...
	 * Stores the source object that this object copied from 
	 *
	 * @param      BatchJobLock $copiedFrom Clone of current object.
	 */
	public function setCopiedFrom(BatchJobLock $copiedFrom)
	{
	 ...
	 *
	 * @return     BatchJobLockPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new BatchJobLockPeer();
		}
		return self::$peer;
	}
	...
	public function getBatchJob(PropelPDO $con = null)
	{
		if ($this->aBatchJob === null && ($this->batch_job_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aBatchJob->addBatchJobLocks($this);
			 */
		}
	...
	/**
	 * Clears out the collBatchJobs collection (array).
	 *
	 ...
	 * @see        addBatchJobs()
	 */
	public function clearBatchJobs()
	{
		$this->collBatchJobs = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collBatchJobs collection (array).
	 *
	 * By default this just sets the collBatchJobs collection to an empty array (like clearcollBatchJobs());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initBatchJobs()
	{
		$this->collBatchJobs = array();
	}

	/**
	 * Gets an array of BatchJob objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this BatchJobLock has previously been saved, it will retrieve
	 * related BatchJobs from storage. If this BatchJobLock is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array BatchJob[]
	 * @throws     PropelException
	 */
	public function getBatchJobs($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(BatchJobLockPeer::DATABASE_NAME);
		}
		...
		if ($this->collBatchJobs === null) {
			if ($this->isNew()) {
			   $this->collBatchJobs = array();
				...
				$this->collBatchJobs = BatchJobPeer::doSelect($criteria, $con);
			}
		} else {
		...
		$this->lastBatchJobCriteria = $criteria;
		return $this->collBatchJobs;
	}
	...
	/**
	 * Method called to associate a BatchJob object to this object
	 * through the BatchJob foreign key attribute.
	 *
	 * @param      BatchJob $l BatchJob
	 * @return     void
	 * @throws     PropelException
	 */
	public function addBatchJob(BatchJob $l)
	{
		if ($this->collBatchJobs === null) {
			$this->initBatchJobs();
		}
		if (!in_array($l, $this->collBatchJobs, true)) { // only add it if the **same** object is not already associated
			array_push($this->collBatchJobs, $l);
			$l->setBatchJobLock($this);
		}
	}
	...
	{
		if ($deep) {
			if ($this->collBatchJobs) {
				foreach ((array) $this->collBatchJobs as $o) {
