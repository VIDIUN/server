...
abstract class BasePermission extends BaseObject  implements Persistent {
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$affectedObjects = PermissionPeer::doUpdate($this, $con);
					if($affectedObjects)
						$this->objectSaved = true;
	...
	public function buildPkeyCriteria()
	{
		...
		if($this->alreadyInSave && count($this->modifiedColumns) == 2 && $this->isColumnModified(PermissionPeer::UPDATED_AT))
		{
			$theModifiedColumn = null;
			foreach($this->modifiedColumns as $modifiedColumn)
				if($modifiedColumn != PermissionPeer::UPDATED_AT)
					$theModifiedColumn = $modifiedColumn;
					
			$atomicColumns = PermissionPeer::getAtomicColumns();
