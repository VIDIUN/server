...
<xsl:stylesheet version="1.0" xmlns:media="http://search.yahoo.com/mrss/" xmlns:atom="http://www.w3.org/2005/Atom" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:php="http://php.net/xsl" exclude-result-prefixes="xs">
		...
		<item>
			...
			<media:content>
				...
				<xsl:attribute name="url">
					...
					<xsl:value-of select="concat ('http://cdnapi.vidiun.com/p/', $partnerId, '/sp/', $partnerId, '00/playManifest/entryId/', $entryId, '/format/url/protocol/http')" />
			...
			<xsl:element name="media:thumbnail">
				...
				<xsl:attribute name="width">256</xsl:attribute>

				<xsl:attribute name="height">144</xsl:attribute>

				<xsl:attribute name="url">

					<xsl:value-of select="concat (thumbnailUrl/@url, '/width/256/height/144')" />
			...
			<xsl:element name="media:keywords">
				...
				<xsl:call-template name="implodecategory">

					<xsl:with-param name="items" select="category/@name" />
	...
	<xsl:template name="implodecategory">
