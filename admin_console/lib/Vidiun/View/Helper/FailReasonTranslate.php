 ...
 * @subpackage views
 */
class Vidiun_View_Helper_FailReasonTranslate extends Zend_View_Helper_Abstract
{
...
//		4 => 'Not enough memory was available. (Added in 7.12.0) ',
	...
	private static $clientDescriptions = array(
		Vidiun_Client_Exception::ERROR_UNSERIALIZE_FAILED => 'Failed to unserialize response',
		Vidiun_Client_Exception::ERROR_FORMAT_NOT_SUPPORTED => 'Unsupported format',
		Vidiun_Client_Exception::ERROR_UPLOAD_NOT_SUPPORTED => 'Upload is not supported',
		Vidiun_Client_Exception::ERROR_CONNECTION_FAILED => 'HTTP connection failed',
		Vidiun_Client_Exception::ERROR_READ_FAILED => 'Response read failed',
		Vidiun_Client_Exception::ERROR_INVALID_PARTNER_ID => 'Invalid partner ID',
		Vidiun_Client_Exception::ERROR_INVALID_OBJECT_TYPE => 'Invalid object type',
	);
	
	public function failReasonTranslate($status, $errType, $errNumber, $message)
	{
		if($status != Vidiun_Client_Enum_BatchJobStatus::FAILED)
			return $this->view->enumTranslate('Vidiun_Client_Enum_BatchJobStatus', $status);
			
		$errTypeDesc = $this->view->enumTranslate('Vidiun_Client_Enum_BatchJobErrorTypes', $errType);
		$ret = $errTypeDesc;
		
		switch($errType)
		{
			case Vidiun_Client_Enum_BatchJobErrorTypes::APP:
				$title = $this->view->enumTranslate('Vidiun_Client_Enum_BatchJobAppErrors', $errNumber);
				$ret = "<span title=\"$title\">$errTypeDesc ($errNumber)</span>";
				break;
				
			case Vidiun_Client_Enum_BatchJobErrorTypes::HTTP:
			...
			case Vidiun_Client_Enum_BatchJobErrorTypes::CURL:
			...
			case Vidiun_Client_Enum_BatchJobErrorTypes::RUNTIME:
				$ret = "<span title=\"$message\">$errTypeDesc ($errNumber)</span>";
				break;
				
			case Vidiun_Client_Enum_BatchJobErrorTypes::VIDIUN_API:
				$ret = "<span title=\"$message\">$errTypeDesc ($errNumber)</span>";
				break;
				
			case Vidiun_Client_Enum_BatchJobErrorTypes::VIDIUN_CLIENT:
				$title = $message;
